...
```{r plot_barras}
data %>% ggplot(aes(x=island,fill=species)) + geom_bar() +theme_bw()  +labs(y="Conteo", x="Isla", fill="Especie", title="Número de individuos muestreados en cada isla")
...
```{r plot_multipanel}
data %>%ggplot(aes(y=body_mass_g,x=sex,fill=species)) + geom_boxplot() + facet_wrap(vars(species)) +theme_bw() + labs(y="Tamaño (g)", x="Sexo") +theme(legend.position = "off")
```
